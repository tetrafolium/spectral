/**
 * This script generates a list of assets that are needed to load the built-in
 * rulesets. It contains all custom functions and *resolved* rulesets. The
 * assets are stored in a single file named `assets.json` in the following
 * format:
 * `<require-call-path>: <content>`
 * where the `require-call-path` is the path you'd normally pass to require(),
 * i.e. `@stoplight/spectral/rulesets/oas/index.js` and `content` is the text
 * data. Assets can be loaded using Spectral#registerStaticAssets static method,
 * i.e.
 * `Spectral.registerStaticAssets(require('@stoplight/spectral/rulesets/assets/assets.json'))`;
 * If you execute the code above, ruleset will be loaded fully offline, without
 * a need to make any request.
 */
