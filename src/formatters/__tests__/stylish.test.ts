...
const oas3SchemaErrors =
    sortResults(require('./__fixtures__/oas3-schema-errors.json'));
...
describe('Stylish formatter', () => {
  test('should prefer message for oas-schema errors', () => {
    ...
    expect(result).toContain(
        'oas3-schema  should NOT have additional properties: type\n');
    expect(result).toContain(
        'oas3-schema  should match exactly one schema in oneOf');
    expect(result).toContain(
        "oas3-schema  should have required property '$ref'\n");
  });
  ...
  3:10         ${
        chalk.white(
            'hint')}  info-contact            Info object should contain \`contact\` object.
  3:10      ${
        chalk.yellow(
            'warning',
            )}  info-description        OpenAPI object info \`description\` must be present and non-empty string.
  5:14        ${
        chalk.red('error')}  info-matches-stoplight  Info must contain Stoplight
 17:13  ${
        chalk.blue(
            'information',
