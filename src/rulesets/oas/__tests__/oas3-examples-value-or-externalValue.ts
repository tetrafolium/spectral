...
describe('oas3-examples-value-or-externalValue', () => {
  ...
  s.setRules({
    'oas3-examples-value-or-externalValue' :
        Object.assign(ruleset.rules['oas3-examples-value-or-externalValue'], {
          recommended : true,
          type : RuleType[ruleset.rules['oas3-examples-value-or-externalValue']
                              .type],
        }),
  });
  ...
  test(
      'multiple examples - return warnings if missing externalValue and value in one',
      async () => {
        const results = await s.run({
          components : {
            examples : {
  ...
  test(
      'multiple examples - return warnings if both externalValue and value in one (in components)',
      async () => {
        const results = await s.run({
          components : {
            examples : {
  ...
  test(
      'multiple examples - return warnings if both externalValue and value in one (in headers)',
      async () => {
        const results = await s.run({
          components : {
            headers : {
              headerName : {
                examples : {
                  first : {value : 'value1'},
                  second :
                      {externalValue : 'external-value2', value : 'value2'},
                  third : {externalValue : 'external-value3'},
                },
              },
            },
        ...
        expect(results).toEqual([
          {
            ...
            message :
                'Examples should have either a `value` or `externalValue` field.',
            path :
                [ 'components', 'headers', 'headerName', 'examples', 'second' ],
            range : {
              end : {
                character : 29,
                line : 10,
  ...
  test(
      'multiple examples - return warnings if both externalValue and value in one (in parameters)',
      async () => {
        const results = await s.run({
          components : {
            parameters : {
        ...
        expect(results).toEqual([
          {
            ...
            message :
                'Examples should have either a `value` or `externalValue` field.',
            path : [
              'components', 'parameters', 'parameterName', 'examples', 'second'
            ],
            range : {
              end : {
                character : 29,
  ...
  test(
      'multiple examples - return warnings if both externalValue and value in one (in content)',
      async () => {
        const results = await s.run({
          paths : {
            '/path' : {
