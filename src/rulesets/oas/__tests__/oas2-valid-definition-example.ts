...
describe('oas2-valid-definition-example', () => {
  ...
  beforeEach(() => {
    ...
    s.setRules({
      'oas2-valid-definition-example' :
          Object.assign(ruleset.rules['oas2-valid-definition-example'], {
            recommended : true,
            type :
                RuleType[ruleset.rules['oas2-valid-definition-example'].type],
          }),
    });
  });
      ...
      async () => {
        ...
        expect(results).toEqual([
          expect.objectContaining({
            ...
            message : '`example` property type should be string',
            path : [
              'definitions', '0', 'xoxo', 'properties', 'a', 'properties', 'b',
  ...
  test(
      'will not fail if an actual property is called example and there is also type/format property',
      async () => {
        const results = await s.run({
          definitions : {
            xoxo : {
