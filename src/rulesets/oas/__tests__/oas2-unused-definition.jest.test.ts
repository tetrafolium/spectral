  ...
  describe('reports unreferenced definitions', () => {
    ...
    test(
        ...
        async () => {
          const fixturePath = path.join(
              __dirname,
              '../../__tests__/__fixtures__/unusedDefinition.remoteLocal.json');

          const doc = await readParsable(fixturePath, {encoding : 'utf8'});
          const results =
              await s.run(new Document(doc, Parsers.Json, fixturePath));
    ...
    test(
        ...
        async () => {
          const fixturePath = path.join(
              __dirname,
              '../../__tests__/__fixtures__/unusedDefinition.indirect.1.json');
