...
describe('unusedComponentsSchema - Http and fs remote references', () => {
  ...
  describe('reports unreferenced components schemas', () => {
    ...
    test(
        'when analyzing a directly self-referencing document from the filesystem',
        async () => {
          const fixturePath = path.join(
              __dirname,
              '../../__tests__/__fixtures__/unusedComponentsSchema.remoteLocal.json');

          const spec = await readParsable(fixturePath, {encoding : 'utf8'});
          const results =
              await s.run(new Document(spec, Parsers.Json, fixturePath));

          expect(results).toEqual([]);
        });

    test(
        'when analyzing an indirectly self-referencing document from the filesystem',
        async () => {
          const fixturePath = path.join(
              __dirname,
              '../../__tests__/__fixtures__/unusedComponentsSchema.indirect.1.json');
