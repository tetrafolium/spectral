...
// impossible to detect the most appropriate error, i.e. oneOf consisting of
// more than 3 members, etc. note,  more errors can be included if certain
...
// The function removes irrelevant (aka misleading, confusing, useless, whatever
// you call it) errors. There are a few exceptions, i.e. security components I
// covered manually, yet apart from them we usually deal with a relatively
// simple scenario that can be literally expressed as: "either proper value of
// $ref property". The $ref part is never going to be interesting for us,
// because both oas-schema rules operate on resolved content, so we won't have
// any $refs left. As you can see, what we deal here wit is actually not really
// oneOf anymore - it's always the first member of oneOf we match against. That
// being said, we always strip both oneOf and $ref, since we are always
// interested in the first error.
