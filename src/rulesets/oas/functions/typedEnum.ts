...
export const typedEnum: IFunction = function(this: IFunctionContext, targetVal,
                                             opts, paths, otherValues) {
  if (targetVal === null || typeof targetVal !== 'object') {
    return;
  }
  if (targetVal.enum === null || targetVal.enum === void 0 ||
      targetVal.type === null || targetVal.type === void 0) {
    return;
  }
  // do not use rest spread operator here, as this causes the whole tslib gets
  // injected despite proper target set... obviously, having tslib inlined makes
  // the code size quite larger (around 4x after compression - 1.8K vs 7.4K).
  ...
  return incorrectValues.map(bad => {
    return {
      message : `Enum value \`${
          bad.val}\` does not respect the specified type \`${type}\`.`,
