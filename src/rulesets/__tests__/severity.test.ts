...
describe('Ruleset severity', () => {
  describe('getDiagnosticSeverity', () => {
    ...
    ])('should successfully match %s human readable severity',
       (human, severity) => {
         expect(getDiagnosticSeverity(human as HumanReadableDiagnosticSeverity))
             .toEqual(severity);
       });
  ...
  describe('getSeverityLevel', () => {
    describe('given invalid rule', () => {
      test('should return provided severity', () => {
        expect(getSeverityLevel({}, 'foo', DiagnosticSeverity.Information))
            .toEqual(DiagnosticSeverity.Information);
        expect(getSeverityLevel({foo : false}, 'foo', DiagnosticSeverity.Hint))
            .toEqual(DiagnosticSeverity.Hint);
      });

      test('should override existing severity level if rule is incomplete',
           () => {
             expect(getSeverityLevel({foo : DiagnosticSeverity.Error}, 'foo',
                                     DiagnosticSeverity.Information))
                 .toEqual(
                     DiagnosticSeverity.Information,
                 );
      ...
      test.each([ 'recommended', 'all' ] as FileRulesetSeverity[])(
          ...
          flag => {
            expect(getSeverityLevel({foo : DiagnosticSeverity.Warning}, 'foo',
                                    flag))
                .toEqual(
                    DiagnosticSeverity.Warning,
                );
            expect(getSeverityLevel({foo : 'error'}, 'foo', flag))
                .toEqual(DiagnosticSeverity.Error);
            expect(getSeverityLevel({foo : true}, 'foo', flag))
                .toEqual(DEFAULT_SEVERITY_LEVEL);
          },
      );
      ...
      test.each([ 'recommended', 'all' ] as FileRulesetSeverity[])(
          ...
          flag => {
            expect(getSeverityLevel({}, 'foo', flag))
                .toEqual(DEFAULT_SEVERITY_LEVEL);
          },
      );
    });
