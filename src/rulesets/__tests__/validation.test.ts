...
describe('Ruleset Validation', () => {
  it('given primitive type should throw', () => {
    expect(assertValidRuleset.bind(null, null))
        .toThrow('Provided ruleset is not an object');
    expect(assertValidRuleset.bind(null, 2))
        .toThrow('Provided ruleset is not an object');
    expect(assertValidRuleset.bind(null, 'true'))
        .toThrow('Provided ruleset is not an object');
  });

  it('given object with no rules and no extends properties should throw',
     () => {
       expect(assertValidRuleset.bind(null, {}))
           .toThrow('Ruleset must have rules or extends property');
       expect(assertValidRuleset.bind(null, {
         rule : {}
       })).toThrow('Ruleset must have rules or extends property');
     });
  ...
  it('given invalid ruleset should throw', () => {
    expect(assertValidRuleset.bind(null, invalidRuleset))
  ...
  it.each([ 'error', 'warn', 'info', 'hint', 'off' ])(
      'recognizes human-readable %s severity', severity => {
        expect(
  ...
  it.each([ [ 2, 'a' ], 2, [ '' ] ])(
      'recognizes invalid ruleset formats syntax', formats => {
        expect(
            assertValidRuleset.bind(null, {
              formats,
              rules : {},
  ...
  it.each([ [ 2, 'a' ], 2, [ '' ] ])('recognizes invalid rule formats syntax',
                                     formats => {
                                       expect(
                                           assertValidRuleset.bind(null, {
                                             rules : {
  ...
  describe('Exceptions validation', () => {
    ...
    it.each(rulesetsWithInvalidExceptStructures)(
        'throws when defined "except" do not match schema', ruleset => {
          expect(() => { assertValidRuleset(ruleset); })
              .toThrow(ValidationError);
        });
  });
  ...
  describe('then validation', () => {
    ...
    describe('casing function', () => {
      it.each([
        ...
        {type : 'snake', disallowDigits : true, separator : {char : 'a'}},
        {
          ...
          separator : {char : 'b', allowLeading : true}
        },
    ...
    describe('enumeration function', () => {
      ...
      it.each([ [ null ], 2, null ])('complains about invalid values %s',
                                     functionOptions => {
                                       expect(
                                           assertValidRuleset.bind(null, {
                                             rules : {
    ...
    describe('length function', () => {
      it.each([ {min : 2}, {max : 4}, {min : 2, max : 4} ])(
          'given valid then %s', functionOptions => {
            expect(
                assertValidRuleset.bind(null, {
                  rules : {
    ...
    describe('pattern function', () => {
      it.each([
        {match : 'foo'}, {notMatch : 'foo'}, {match : 'foo', notMatch : 'bar'}
      ])(
          'given valid then %s',
          functionOptions => {
            expect(
      ...
      it.each(
          [ {match : 2}, {notMatch : null}, {match : 4, notMatch : 'bar'} ])(
          'complains about invalid options %s',
          functionOptions => {
            expect(
                assertValidRuleset.bind(null, {
    ...
    describe('unreferencedReusableObject function', () => {
      ...
      it.each([ 2, '', 'd' ])(
          'complains about invalid options %s', reusableObjectsLocation => {
            expect(
                assertValidRuleset.bind(null, {
                  rules : {
                    rule : {
    ...
    describe('xor function', () => {
      ...
      it.each([ [ null ], 2, null ])('complains about invalid properties %s',
                                     functionOptions => {
                                       expect(
                                           assertValidRuleset.bind(null, {
                                             rules : {
...
describe('Function Validation', () => {
  ...
  it('does not call supplied fn if options do not meet schema', () => {
    ...
    expect(
        () => wrapped({}, {}, {given : []}, {original : [], given : []} as any))
        .toThrow(ValidationError);
  });
  ...
  it('calls supplied fn and passes all other arguments if options do match schema',
     () => {
       ...
       expect(fn).toHaveBeenCalledWith({}, '2', {given : []},
                                       {original : [], given : []});
     });
});
