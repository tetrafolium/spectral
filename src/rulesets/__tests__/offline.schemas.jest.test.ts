...
interface ITestCases {
  [rulesetname: string]: {
    fixtures: Array<
        {fixture : string; format : {name : string; lookupFn : FormatLookup};}>;
  };
}
...
const flattenedTestCases =
    Object.entries(knownRulesets)
        .reduce<FlattenedTestCases[]>((flattened, [ key, testCase ]) => {
          for (const f of testCase.fixtures) {
            flattened.push(
                [ key, f.fixture, f.format.name, f.format.lookupFn ]);
          }
          return flattened;
        }, []);
...
describe('Online vs Offline context', () => {
  ...
  test.each(flattenedTestCases)(
      ...
      async (ruleset: string, fixture: string, formatName: string,
        ...
        const resolvingSpectral =
            new Spectral({resolver : httpAndFileResolver});
  ...
  test('all rulesets are accounted for', async () => {
    ...
    // Will fail when a ruleset has not been added to the `knownRulesets`
    // variable
    expect(ordinalSort(discoveredRulesets))
        .toEqual(ordinalSort(Object.keys(knownRulesets)));
  });
});
