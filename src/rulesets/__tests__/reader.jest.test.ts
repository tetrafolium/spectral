...
const validFlatRuleset =
    path.join(__dirname, './__fixtures__/valid-flat-ruleset.json');
const validRequireInfo =
    path.join(__dirname, './__fixtures__/valid-require-info-ruleset.yaml');
const github447 =
    path.join(__dirname, './__fixtures__/github-issue-447-fixture.yaml');
const enabledAllRuleset =
    path.join(__dirname, './__fixtures__/enable-all-ruleset.json');
const invalidRuleset =
    path.join(__dirname, './__fixtures__/invalid-ruleset.json');
const extendsOnlyRuleset =
    path.join(__dirname, './__fixtures__/extends-only-ruleset.json');
const extendsAllOasRuleset =
    path.join(__dirname, './__fixtures__/extends-oas-ruleset.json');
const extendsUnspecifiedOasRuleset =
    path.join(__dirname, './__fixtures__/extends-unspecified-oas-ruleset.json');
const extendsDisabledOasRuleset =
    path.join(__dirname, './__fixtures__/extends-disabled-oas-ruleset.yaml');
const extendsOasWithOverrideRuleset = path.join(
    __dirname, './__fixtures__/extends-oas-with-override-ruleset.json');
const extendsRelativeRuleset =
    path.join(__dirname, './__fixtures__/extends-relative-ruleset.json');
const myOpenAPIRuleset =
    path.join(__dirname, './__fixtures__/my-open-api-ruleset.json');
const extendsNPMRuleset =
    path.join(__dirname, './__fixtures__/ruleset-extends-npm.json');
const extendsNPMVersionedRuleset =
    path.join(__dirname, './__fixtures__/ruleset-extends-npm-versioned.json');
const fooRuleset = path.join(__dirname, './__fixtures__/foo-ruleset.json');
const customFunctionsDirectoryRuleset = path.join(
    __dirname, './__fixtures__/custom-functions-directory-ruleset.json');
const rulesetWithMissingFunctions =
    path.join(__dirname, './__fixtures__/ruleset-with-missing-functions.json');
const fooExtendsBarRuleset =
    path.join(__dirname, './__fixtures__/foo-extends-bar-ruleset.json');
const selfExtendingRuleset =
    path.join(__dirname, './__fixtures__/self-extending-ruleset.json');
const simpleDisableRuleset =
    path.join(__dirname, './__fixtures__/simple-disable-ruleset.yaml');
const standaloneExceptRuleset =
    path.join(__dirname, './__fixtures__/exceptions/standalone.yaml');
const simpleExceptRuleset =
    path.join(__dirname, './__fixtures__/exceptions/simple.yaml');
const inheritingExceptRuleset =
    path.join(__dirname, './__fixtures__/exceptions/inheriting.yaml');
const invalidExceptRuleset =
    path.join(__dirname, './__fixtures__/exceptions/invalid.yaml');
const fooCJSFunction = fs.readFileSync(
    path.join(__dirname, './__fixtures__/functions/foo.cjs.js'), 'utf8');
const barFunction = fs.readFileSync(
    path.join(__dirname, './__fixtures__/customFunctions/bar.js'), 'utf8');
const truthyFunction = fs.readFileSync(
    path.join(__dirname, './__fixtures__/customFunctions/truthy.js'), 'utf8');
...
describe('Rulesets reader', () => {
  ...
  it('given flat, valid ruleset file should return rules', async () => {
    expect(await readRuleset(validFlatRuleset))
        .toEqual(
  ...
  it('given two flat, valid ruleset files should return ruleset with rules',
     async () => {
       expect(await readRuleset([ validFlatRuleset, validRequireInfo ]))
           .toEqual(
               expect.objectContaining({
                 rules : {
                   'valid-rule' : {
  ...
  it('given ruleset  with no custom rules extending other rulesets',
     async () => {
  ...
  it('should inherit properties of extended rulesets', async () => {
    ...
    // we pick up *all* rules only from spectral:oas and keep their severity
    // level or set a default one
    expect(rules).toEqual(
        expect.objectContaining({
          ...Object.entries(oasRulesetRules)
              .reduce<Dictionary<IRule, string>>(
                  (oasRules, [ name, rule ]) => {
                    oasRules[name] = {
                      ...rule,
                      formats : expect.arrayContaining([ expect.any(String) ]),
                      ...(rule.severity === void 0 &&
                          {severity : DiagnosticSeverity.Warning}),
  ...
  it('should inherit properties of extended rulesets and disable not recommended ones',
     () => {
       return expect(readRuleset(extendsUnspecifiedOasRuleset))
           .resolves.toEqual(
               expect.objectContaining({
                           ...
                           (rules, [ name, rule ]) => {
                             rules[name] = {
                               ...
                               ...(rule.severity === undefined &&
                                   {severity : DiagnosticSeverity.Warning}),
                               ...(rule.recommended === false &&
                                   {severity : -1}),
                               ...(rule.recommended === void 0 &&
                                   {recommended : true}),
                               then : expect.any(Object),
                             };
  ...
  it('should always disable a rule with false severity', () => {
    return expect(readRuleset(simpleDisableRuleset))
  ...
  it('given GitHub issue #447, loads recommended oas3 and oas rules correctly',
     async () => {
                 ...
                 .reduce<Dictionary<IRule, string>>(
                     ...
                     },
                     {
                       'schema-names-pascal-case' : {
                         description :
                             'Schema names MUST be written in PascalCase',
                       ...
                       'operation-id-kebab-case' : {
                         description :
                             'operationId MUST be written in kebab-case',
                         given :
  ...
  it('should set severity of disabled rules to off', () => {
    return expect(readRuleset(extendsDisabledOasRuleset))
        .resolves.toHaveProperty(
            'rules',
            expect.objectContaining({
              ...Object.entries(oasRuleset.rules)
                  .reduce<Dictionary<unknown>>(
                      (rules, [ name, rule ]) => {
  ...
  it('should override properties of extended rulesets', () => {
    return expect(readRuleset(extendsOasWithOverrideRuleset))
        .resolves.toHaveProperty('rules.operation-2xx-response', {
  ...
  it('should prefer top-level ruleset severity level', async () => {
    ...
    expect(
        Object.values(enabledRules)
            .filter(
                rule => rule.severity === -1 || rule.severity === 'off' ||
                        rule.severity === undefined,
                ),
        )
        .toHaveLength(0);
  ...
  it('given spectral:oas ruleset, should not pick up unrecommended rules',
     () => {
       return expect(readRuleset('spectral:oas'))
           .resolves.toEqual(
               expect.objectContaining({
                 rules : expect.objectContaining({
  ...
  it('given ruleset with extends set to all, should enable all rules', () => {
    return expect(
               readRuleset(path.join(
                   __dirname,
                   './__fixtures__/inheritanceRulesets/my-ruleset.json')),
               )
  ...
  it('given ruleset with extends set to recommended, should enable recommended rules',
     () => {
       return expect(
                  readRuleset(path.join(
                      __dirname,
                      './__fixtures__/inheritanceRulesets/my-ruleset-recommended.json')),
                  )
           .resolves.toStrictEqual({
             exceptions : {},
             functions : {},
  ...
  it('given ruleset with extends set to off, should disable all rules', () => {
    return expect(
               readRuleset(path.join(
                   __dirname,
                   './__fixtures__/inheritanceRulesets/ruleset-c.json')),
               )
        .resolves.toStrictEqual({
          exceptions : {},
          functions : {},
  ...
  it('should support local rulesets', () => {
    return expect(readRuleset(extendsRelativeRuleset))
        .resolves.toEqual(
            expect.objectContaining({
              rules : {
                PascalCase : {
    ...
    return expect(readRuleset(extendsNPMRuleset)).resolves.toEqual({
      ...
      functions : {
        min : {
          ...
          schema : null,
          source :
              'https://unpkg.com/example-spectral-ruleset/functions/min.js',
        },
        'random-id-0' : {
          code : minFnCode,
          name : 'min',
       ...
       return expect(readRuleset(extendsNPMVersionedRuleset)).resolves.toEqual({
         ...
         functions : {
           min : {
             ...
             schema : null,
             source :
                 'https://unpkg.com/example-spectral-ruleset@0.0.3/functions/min.js',
           },
           'random-id-0' : {
             code : minFnCode,
             name : 'min',
  ...
  it('given a ruleset with custom functions should return rules and resolved functions',
     async () => {
        ...
        .toEqual(
            expect.objectContaining({
              bar : {
                ...
                schema : null,
                source : path.join(customFunctionsDirectoryRuleset,
                                   '../customFunctions/bar.js'),
              },
              truthy : {
                name : 'truthy',
                ref : expect.stringMatching(/^random-id-[01]$/),
                schema : null,
                source : path.join(customFunctionsDirectoryRuleset,
                                   '../customFunctions/truthy.js'),
              },
            }),
        );

    const [barRandomName, barFunctionDef] =
        Object.entries(ruleset.functions)
            .find(
                ([ name ]) => ruleset.functions.bar.ref === name,
                )!;
    const [truthyRandomName, truthyFunctionDef] =
        Object.entries(ruleset.functions)
            .find(
  ...
  it('given non-existent ruleset should output error', () => {
    ...
    return expect(readRuleset('oneParentRuleset'))
  ...
  it('should reject if request is not finished within a specified timeout',
     () => {
       ...
       return expect(ruleset).rejects.toThrowError(
           'Could not parse https://unpkg.com/oneParentRuleset: Timeout');
     });

  it('given invalid ruleset should output errors', () => {
    return expect(readRuleset(invalidRuleset))
        .rejects.toThrowError(/should have required property/);
  });
  ...
  it('is able to load the whole ruleset from static file', async () => {
    ...
    Spectral.registerStaticAssets(
        require('../../../rulesets/assets/assets.json'));
  ...
  describe('Exceptions loading', () => {
      ...
      expect(Object.entries(ruleset.exceptions)).toEqual([
        [
          expect.stringMatching(
              '/__tests__/__fixtures__/exceptions/one.yaml#$'),
          [ 'my-rule-1' ]
        ],
        [
          expect.stringMatching('/__tests__/__fixtures__/two.yaml#$'),
          [ 'my-rule-2' ]
        ],
        [
          expect.stringMatching(
              '/__tests__/__fixtures__/exceptions/sub/three.yaml#$'),
          [ 'my-rule-3' ]
        ],
      ]);
    });
    ...
    it('should throw when ruleset contains invalid exceptions', () => {
      expect(readRuleset(invalidExceptRuleset))
          .rejects.toThrow('is not a valid uri');
    });

    it('should handle loading a ruleset deriving from a built-in one',
       async () => {
    ...
    it('should handle loading a ruleset deriving from another one',
       async () => {
         ...
         expect(Object.entries(ruleset.exceptions)).toEqual([
           ...
           [
             expect.stringMatching(
                 '/__tests__/__fixtures__/exceptions/four.yaml#$'),
             [ 'my-rule-4' ]
           ],
         ]);
       });
