...
describe('Code evaluators', () => {
  describe('Export evaluator', () => {
    it('supports require', () => {
      expect(evaluateExport(`module.exports = require('./foo')`,
    ...
    it('supports require.resolve', () => {
      expect(
          path.normalize(
              evaluateExport(
                  `module.exports = () => require.resolve('./foo', { paths: ['${
    ...
    it.each([ 'cache', 'extensions' ])('exposes %s', member => {
      expect(
          evaluateExport(`module.exports = () => require['${member}']`, null)())
          .toStrictEqual(require[member]);
    });
  });
});
