...
// let's point at dist directory that has all relevant files (including custom
// functions) transpiled
...
function resolveSpectralVersion(pkg: string): string {
  return SPECTRAL_PKG_VERSION.length === 0
             ? pkg
             : pkg.replace(SPECTRAL_PKG_NAME,
                           `${SPECTRAL_PKG_NAME}@${SPECTRAL_PKG_VERSION}`);
}
...
export function isNPMSource(src: string): boolean {
  return src.startsWith(NPM_PKG_ROOT) &&
         !src.includes(`${NPM_PKG_ROOT}${
             SPECTRAL_PKG_NAME}`); // we ignore spectral on purpose, since they
...
async function resolveFromFS(from: string, to: string): Promise<string> {
  ...
  if (SPECTRAL_SRC_ROOT.length > 0 && SPECTRAL_SRC_ROOT !== '/' &&
      to.startsWith(SPECTRAL_PKG_NAME)) {
    targetPath =
        path.join(SPECTRAL_SRC_ROOT, to.replace(SPECTRAL_PKG_NAME, './'));
  ...
  // if found in static assets, it's fine, as readParsable will handle it just
  // fine
  if (targetPath in STATIC_ASSETS) {
    return targetPath;
  }

  // if it's not a built-in ruleset, try to resolve the file according to the
  // provided path
  if (await exists(targetPath)) {
    return targetPath;
  }
...
export async function findFile(from: string, to: string): Promise<string> {
  ...
  try {
    ...
    // fs lookup failed...
    // we either given a npm module or the code is executed in a browser or
    // other environment without fs access
...
function exists(uri: string): Promise<boolean> {
  return new Promise<boolean>(
      resolve => { fs.stat(uri, err => { resolve(err === null); }); });
}
