...
export type CJSExport = Partial<
    {exports : object | ESCJSCompatibleExport; require : NodeJS.Require;}>;
...
function requireUnavailable() {
  throw new ReferenceError(
      'require() is supported only in the Node.JS environment');
}

function stubRequire(): NodeJS.Require {
  function req() { requireUnavailable(); }
...
function proxyRequire(source: string): NodeJS.Require {
  ...
  return Object.defineProperties(
      req, Object.getOwnPropertyDescriptors(actualRequire));
}

const isRequiredSupported =
    typeof require === 'function' && typeof require.main === 'object' &&
    require.main !== null && 'paths' in require.main && 'cache' in require;
...
const createDefine = (exports: CJSExport) => {
  const define = (nameOrFactory: string|string[]|Function,
                  factory: Function): Optional<CJSExport> => {
...
const isESCJSCompatibleExport = (obj: unknown): obj is ESCJSCompatibleExport =>
    isObject(obj) && 'default' in obj;

// note: this code is hand-crafted and cover cases we want to support
// be aware of using it in your own project if you need to support a variety of
// module systems
export const evaluateExport = (body: string, source: string|null,
                               inject: Dictionary<unknown> = {}): Function => {
  ...
  Function('module', 'exports', 'define', 'require', ...Object.keys(inject),
           String(body))
      .call(
          root,
          mod,
...
export const compileExportedFunction =
    ({code, name, source, schema, inject}: CompileOptions) => {
      const exportedFn = evaluateExport(code, source, inject) as IFunction;

      const fn = schema !== null
                     ? decorateIFunctionWithSchemaValidation(exportedFn, schema)
                     : exportedFn;
