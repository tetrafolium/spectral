...
describe(`Rule '${ruleName}'`, () => {
  ...
  test(
      'return result if components.schemas.{schema}.examples.{position} is not valid against the schema it decorates',
      async () => {
  ...
  test(
      'return result if components.parameters.{parameter}.schema.examples.{position} is not valid against the schema it decorates',
      async () => {
        const clone = cloneDeep(doc);
        ...
        expect(results).toEqual([
          expect.objectContaining({
            ...
            message : '`0` property type should be string',
            path : [
              'components', 'parameters', 'orphanParameter', 'schema',
              'examples', '0'
            ],
            severity : rule.severity,
          }),
  ...
  test(
      'return result if channels.{channel}.parameters.{parameter}.schema.examples.{position} is not valid against the schema it decorates',
      async () => {
        const clone = cloneDeep(doc);
