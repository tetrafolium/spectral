...
describe(`Rule '${ruleName}'`, () => {
  ...
  test(
      'return result if components.messages.{message}.headers is not of type "object"',
      async () => {
  ...
  test(
      'return result if components.messages.{message}.headers lacks "type" property',
      async () => {
        const clone = cloneDeep(doc);
        ...
        expect(results).toEqual([
          expect.objectContaining({
            code : ruleName,
            message :
                'Headers schema type should be `object` (`headers` property should have required property `type`).',
  ...
  test(
      'return result if components.messageTraits.{trait}.headers is not of type "object"',
      async () => {
        const clone = cloneDeep(doc);
        ...
        expect(results).toEqual([
          expect.objectContaining({
            ...
            message :
                'Headers schema type should be `object` (`type` property should be equal to one of the allowed values: object. Did you mean object?).',
            path :
                [ 'components', 'messageTraits', 'aTrait', 'headers', 'type' ],
            severity : rule.severity,
          }),
        ]);
      });
  ...
  test(
      'return result if components.messageTraits.{trait}.headers lacks "type" property',
      async () => {
        const clone = cloneDeep(doc);

        clone.components.messageTraits.aTrait.headers = {
          const : 'Hello World!'
        };
  ...
  test.each([ 'publish', 'subscribe' ])(
      ...
      async (property: string) => {
        ...
        clone.channels['users/{userId}/signedUp'][property].message.headers = {
          const : 'Hello World!'
        };
        ...
        expect(results).toEqual([
          expect.objectContaining({
            ...
            message :
                'Headers schema type should be `object` (`headers` property should have required property `type`).',
            path : [
              'channels', 'users/{userId}/signedUp', property, 'message',
              'headers'
            ],
            severity : rule.severity,
          }),
