...
describe(`Rule '${ruleName}'`, () => {
  ...
  test(
      'return result if channels.{channel}.parameters.{parameter} lack a description',
      async () => {
        const clone = cloneDeep(doc);

        delete clone.channels['users/{userId}/signedUp']
  ...
  test('return result if components.parameters.{parameter} lack a description',
       async () => {
         const clone = cloneDeep(doc);
