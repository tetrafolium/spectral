...
describe(`Rule '${ruleName}'`, () => {
  ...
  test(
      'return result if components.messages.{message}.schemaFormat is set to a non supported value',
      async () => {
  ...
  test(
      'return result if components.messageTraits.{trait}.schemaFormat is set to a non supported value',
      async () => {
        const clone = cloneDeep(doc);
  ...
  test.each([ 'publish', 'subscribe' ])(
      ...
      async (property: string) => {
        ...
        clone.channels['users/{userId}/signedUp'][property]
            .message.schemaFormat = 'application/nope';
