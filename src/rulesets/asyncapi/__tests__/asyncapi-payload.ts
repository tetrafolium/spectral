...
describe(`Rule '${ruleName}'`, () => {
  ...
  test(
      'return result if components.messages.{message}.payload is not valid against the AsyncApi2 schema object',
      async () => {
        ...
        expect(results).toEqual([
          expect.objectContaining({
            ...
            path : [
              'components', 'messages', 'aMessage', 'payload', 'deprecated'
            ],
  ...
  test(
      'return result if components.messageTraits.{trait}.payload is not valid against the AsyncApi2 schema object',
      async () => {
        const clone = cloneDeep(doc);
        ...
        expect(results).toEqual([
          expect.objectContaining({
            ...
            message : '`deprecated` property type should be boolean',
            path : [
              'components', 'messageTraits', 'aTrait', 'payload', 'deprecated'
            ],
            severity : rule.severity,
          }),
        ]);
