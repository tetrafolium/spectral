...
export class Rule {
  ...
  public get enabled() { return this.severity !== -1; }

  constructor(name: string, rule: IRule) {
    this.name = name;
    this.description = rule.description ? ? null;
    this.message = rule.message ? ? null;
    this.severity = rule.severity === void 0
                        ? DEFAULT_SEVERITY_LEVEL
                        : getDiagnosticSeverity(rule.severity);
  ...
  public matchesFormat(formats: IDocument['formats']) {
    ...
    return Array.isArray(formats) &&
           hasIntersectingElement(this.formats, formats);
  }
}
