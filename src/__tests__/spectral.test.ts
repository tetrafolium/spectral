...
describe('spectral', () => {
  describe('loadRuleset', () => {
    ...
    ])('should support loading "%s" built-in ruleset',
       async (rulesetName, rules) => {
         const s = new Spectral();
         await s.loadRuleset(rulesetName);

         expect(s.rules).toEqual(
             expect.objectContaining(
                 Object.entries(rules).reduce<RunRuleCollection>(
                     (oasRules, [ name, rule ]) => {
    ...
    test.each([
      ...
      [ 'spectral:asyncapi', asyncApiRulesetRules ],
       ...
       async (rulesetName, expectedRules) => {
         const s = new Spectral();
         await s.loadRuleset([ rulesetName, rulesetName ]);

         expect(s.rules).toEqual(
             Object.entries(expectedRules)
                 .reduce<RunRuleCollection>(
                     (oasRules, [ name, rule ]) => {
          oasRules[name] = expect.objectContaining({
            name,
            given: expect.anything(),
      ...
      test('should recognize the source of local $refs', () => {
        ...
        s.setRules({
          'pagination-responses-have-x-next-token' : {
            description :
                'All collection endpoints have the X-Next-Token parameter in responses',
  ...
  describe('setRuleset', () => {
    ...
    describe('exceptions handling', () => {
      it.each([
        [ 'one.yaml#' ], [ 'one.yaml#/' ], [ 'one.yaml#/toto' ],
        [ 'down/one.yaml#/toto' ], [ '../one.yaml#/toto' ]
      ])(
          ...
          location => {
            ...
            expect(() => {
              s.setRuleset({rules : {}, functions : {}, exceptions});
                ...
                .toThrow(new RegExp(`.+\`${
                    escapeRegExp(
                        location)}\`.+is not a valid uri.+Only absolute Uris are allowed`));
          },
      );
      ...
      it.each([
        ...
        [ 'c:\\one.yaml#/toto', 'c:/one.yaml#/toto' ],
      ])('normalizes absolute locations (location: "%s")',
         (location, expected) => {
           const exceptions = buildRulesetExceptionCollectionFrom(location);
