...
describe('request', () => {
  ...
  describe('when agent is set', () => {
    beforeEach(() => {
      DEFAULT_REQUEST_OPTIONS.agent =
          new ProxyAgent(`http://localhost:${PORT}`);
    });

    afterEach(() => { delete DEFAULT_REQUEST_OPTIONS.agent; });

    describe('loading a ruleset', () => {
      it('proxies the request', () => {
        const s = new Spectral();

        return expect(s.loadRuleset('http://localhost:4000/custom-ruleset'))
    ...
    describe('loading a $ref', () => {
      it('proxies the request', () => {
        ...
        return expect(s.runWithResolved(doc))
            .resolves.toHaveProperty('resolved', {
              info : {
                title : '',
                description : 'Foo',
              },
