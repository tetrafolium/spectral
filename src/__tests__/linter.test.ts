...
const invalidSchema =
    JSON.stringify(require('./__fixtures__/petstore.invalid-schema.oas3.json'));
const studioFixture = JSON.stringify(
    require('./__fixtures__/studio-default-fixture-oas3.json'), null, 2);
const todosInvalid =
    JSON.stringify(require('./__fixtures__/todos.invalid.oas2.json'));
const petstoreMergeKeys =
    JSON.stringify(require('./__fixtures__/petstore.merge.keys.oas3.json'));
...
describe('linter', () => {
  ...
  test('should respect the format of data and run rules associated with it',
       async () => {
         spectral.registerFormat('foo-bar', obj => typeof obj === 'object' &&
                                                   obj !== null &&
                                                   'foo-bar' in obj);
  ...
  test(
      'should not run any rule with defined formats if there are no formats registered',
      async () => {
        spectral.setRules({
          rule1 : {
            given : '$.x',
        ...
        expect(result).toEqual([
          expect.objectContaining({
            ...
            message :
                'The provided document does not match any of the registered formats [oas2, oas3]',
          }),
          expect.objectContaining({
            code : 'rule3',
          }),
  ...
  test(
      'should not run any rule with defined formats if some formats are registered but document format could not be associated',
      async () => {
        spectral.registerFormat('foo-bar', obj => typeof obj === 'object' &&
                                                  obj !== null &&
                                                  'foo-bar' in obj);
        ...
        expect(result).toEqual([
          {
            message :
                'The provided document does not match any of the registered formats [oas2, oas3, foo-bar]',
            path : [],
            range : expect.any(Object),
            severity : DiagnosticSeverity.Warning,
            code : 'unrecognized-format',
  ...
  test('given ignoreUnknownFormat, should not warn about unmatched formats',
       async () => {
         spectral.registerFormat('foo-bar', obj => typeof obj === 'object' &&
                                                   obj !== null &&
                                                   'foo-bar' in obj);
...
`;
  ...
  test(
      'should report a valid line number for json paths containing escaped slashes',
      async () => {
        spectral.registerFormat('oas2', isOpenApiv2);
        spectral.registerFormat('oas3', isOpenApiv3);
        await spectral.loadRuleset('spectral:oas');
    ...
    expect(result).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            ...
            path : [
              'paths', '/todos/{todoId}', 'put', 'parameters', '1', 'schema',
              '$ref'
            ],
            severity : DiagnosticSeverity.Error,
          }),
  ...
  test(
      'should report when a resolver is not defined for a given $ref type',
      async () => {
        ...
        expect(result).toEqual([
          expect.objectContaining({
            code : 'invalid-ref',
            message :
                "No resolver defined for scheme 'file' in ref ./models/pet.yaml",
            path : [
          ...
          expect.objectContaining({
            code : 'invalid-ref',
            message :
                "No resolver defined for scheme 'file' in ref ../common/models/error.yaml",
            path : [
  ...
  describe('functional tests for the given property', () => {
    ...
    describe('when given path is set', () => {
      test('should pass given path through to lint function', async () => {
        ...
        let given: unknown;
        (fakeLintingFunction as jest.Mock)
            .mockImplementation((_targetVal, _opts, paths, values) => {
              path = [...paths.given ];
              given = values.given;
            });
      ...
      test(
          'given array of paths, should pass each given path through to lint function',
          async () => {
            spectral.setRules({
              example : {
                message : '',
            ...
            expect(fakeLintingFunction.mock.calls[0][3].given)
                .toEqual(target.responses);
            expect(fakeLintingFunction.mock.calls[1][2].given).toEqual([
              'responses', '200'
            ]);
            expect(fakeLintingFunction.mock.calls[1][3].given)
                .toEqual(target.responses['200']);
          });
    });
  ...
  describe('functional tests for the then statement', () => {
    ...
    describe('given list of then objects', () => {
      test('should call each one with the appropriate args', async () => {
        ...
        expect(fakeLintingFunction2.mock.calls[0][0])
            .toEqual(target.responses['200']);
    ...
    test('should print primitive values', () => {
      ...
      spectral.setRules({
        'header-parameter-names-kebab-case' : {
          ...
          recommended : true,
          description :
              'A parameter in the header should be written in kebab-case',
      ...
      return expect(
          ...
          .resolves.toEqual([
            expect.objectContaining({
              ...
              message :
                  '"fooA" is not kebab-cased: must match the pattern \'^[a-z0-9]+((-[a-z0-9]+)+)?$\'',
            ...
            expect.objectContaining({
              code : 'header-parameter-names-kebab-case',
              message :
                  '"d 1" is not kebab-cased: must match the pattern \'^[a-z0-9]+((-[a-z0-9]+)+)?$\'',
  ...
  test('should evaluate {{path}} in validation messages', async () => {
    ...
    return expect(spectral.run(invalidSchema))
        .resolves.toEqual(
            expect.arrayContaining([
              expect.objectContaining({
                code : 'oas3-schema',
                message :
                    'Schema error at #/paths/~1pets/get/responses/200/headers/header-1',
                path : [
  ...
  describe('runWithResolved', () => {
    test('should include both resolved and validation results', async () => {
      ...
      const {resolved, results} =
          await spectral.runWithResolved(petstoreMergeKeys);
  ...
  describe('legacy parsed document', () => {
    ...
    test(
        'given missing source on parsedResult, should try to set resolveUri as source of the document',
        async () => {
          const parsedResult: IParsedResult = {
            parsed : {
              data : {},
  ...
  test.each([ '1', 'null', '', 'false' ])(
      'given %s input, should report nothing', async input => {
        ...
        const results = await s.run(
            doc,
            {ignoreUnknownFormat : true, resolve : {documentUri : source}});
  ...
  test('should be capable of linting arrays', async () => {
    ...
    const results = await s.run(
        doc, {ignoreUnknownFormat : true, resolve : {documentUri : source}});
