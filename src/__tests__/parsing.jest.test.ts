...
const implicitStringOAS3Document =
    path.join(__dirname, './__fixtures__/implicit-strings.oas3.yaml');

describe('Parsing', () => {
  it('parses YAML scalar values according to YAML 1.2', () => {
    const {data} =
        parseWithPointers(fs.readFileSync(implicitStringOAS3Document, 'utf8'));
