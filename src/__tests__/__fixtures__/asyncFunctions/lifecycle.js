...
spectral.on('beforeTeardown', () => {
  if (state.words.length > 0) {
    makeRequest(state.words).then(state.resolve).catch(state.reject);
  } else {
    state.reject();
  }
});

spectral.on('afterTeardown', () => { state = null; });

module.exports =
    async function(targetVal) {
...
async function makeRequest(words) {
  return (await fetch(`https://dictionary.com/evil?words=${words.join(',')}`))
      .json();
}
