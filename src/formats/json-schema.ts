...
const $SCHEMA_DRAFT4_REGEX =
    /^https?:\/\/json-schema.org\/draft-04\/(?:hyper-)?schema#?$/;
const $SCHEMA_DRAFT6_REGEX =
    /^https?:\/\/json-schema.org\/draft-06\/(?:hyper-)?schema#?$/;
const $SCHEMA_DRAFT7_REGEX =
    /^https?:\/\/json-schema.org\/draft-07\/(?:hyper-)?schema#?$/;
const $SCHEMA_DRAFT_2019_09_REGEX =
    /^https?:\/\/json-schema.org\/draft\/2019-09\/(?:hyper-)?schema#?$/;

const hasValidJSONSchemaType = (document: object|Partial<{type : unknown}>) =>
    'type' in document && typeof document.type === 'string' &&
    KNOWN_JSON_SCHEMA_TYPES.includes(document.type);

const hasValidJSONSchemaCombiner = (document: object) =>
    KNOWN_JSON_SCHEMA_COMBINERS.some(combiner => combiner in document &&
                                                 isObject(document[combiner]));

export const isJSONSchema =
    (document: unknown): document is JSONSchema&{$schema : string} =>
        isObject(document) && '$schema' in document &&
        String((document as Partial<Pick<JSONSchema, '$schema'>>).$schema)
            .includes('//json-schema.org/');

export const isJSONSchemaLoose = (document: unknown) =>
    isObject(document) &&
    (isJSONSchema(document) || hasValidJSONSchemaType(document) ||
     hasValidJSONSchemaCombiner(document) || 'not' in document);
...
export const isJSONSchemaDraft2019_09 = (document: unknown) =>
    ...
    $SCHEMA_DRAFT_2019_09_REGEX.test(document.$schema);
