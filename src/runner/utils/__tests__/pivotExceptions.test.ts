...
describe('pivotExceptions', () => {
  ...
  it('returns a rule based dictionary', () => {
    ...
    const runRules =
        {a : dummyRule, b : dummyRule, c : dummyRule, d : dummyRule};
    const expected: Dictionary<IExceptionLocation[], string> = {
      a : [
        {source : 'one', path : [ '1' ]},
        {source : 'three', path : [ '3' ]},
  ...
  it.each(malformedLocations)(
      'throws upon detected malformed location (%s)', malformed => {
        const bad = buildRulesetExceptionCollectionFrom(malformed);

        expect(() => { pivotExceptions(bad, {}); }).toThrow(InvalidUriError);
      });
});
