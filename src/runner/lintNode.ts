...
export const lintNode = (
  ...
  const givenPath = node.path.length > 0 && node.path[0] === '$'
                        ? node.path.slice(1)
                        : node.path;
  ...
  for (const then of rule.then) {
    ...
    if (typeof func !== 'function') {
      console.warn(
          `Function ${then.function} not found. Called by rule ${rule.name}.`);
      continue;
    }
    ...
    for (const target of targets) {
      const targetPath =
          target.path.length > 0 ? [...givenPath, ...target.path ] : givenPath;
...
function processTargetResults(
  ...
  for (const result of results) {
    ...
    const path = associatedItem ?.path
        ? ? getClosestJsonPath(context.documentInventory.resolved,
                               escapedJsonPath);
    const source = associatedItem ?.document.source;

    if (exceptionLocations !== void 0 &&
        isAKnownException(path, source, exceptionLocations)) {
      continue;
    }

    const document =
        associatedItem ?.document ? ? context.documentInventory.document;
    const range =
        document.getRangeForJsonPath(path, true) ? ? Document.DEFAULT_RANGE;
    const value = path.length === 0 ? document.data : get(document.data, path);

    const vars: IMessageVars = {
      property : associatedItem
      ?.missingPropertyPath &&
               associatedItem.missingPropertyPath.length > path.length
           ? printPath(
                 associatedItem.missingPropertyPath.slice(path.length - 1),
                 PrintStyle.Dot)
