...
describe('sortResults', () => {
  const shuffleBy = (arr: IRuleResult[], indices: number[]): IRuleResult[] => {
    ...
    const shuffled = results.map((v, i) => ({...v, pos : indices[i]}))
...
describe('compareResults', () => {
  test('should properly order results source', () => {
    const input = {
      ...
      message : '99',
      severity :
          DiagnosticSeverity.Error, // or any other level, it's irrelevant
    };
    ...
    [{one : undefined, another : undefined, expected : 0},
     ...
     {one : 'a', another : 'b', expected : -1},
      ...
      expect(compareResults({...input, source : tc.one},
                            {...input, source : tc.another}))
          .toEqual(tc.expected);
    });
  });
  ...
  test('should properly order results code', () => {
    ...
    [{one : undefined, another : undefined, expected : 0},
     ...
     {one : 2, another : '10', expected : -1},
      ...
      expect(compareResults({...input, code : tc.one},
                            {...input, code : tc.another}))
          .toEqual(tc.expected);
    });
  });
});

const buildPosition = (line: number, char: number):
    IPosition => { return {line, character : char}; };
...
describe('comparePosition', () => {
  ...
  test.each(positionTestCases)(
      ...
      (leftLine, leftChar, rightLine, rightChar, expected) => {
        expect(comparePosition(buildPosition(leftLine, leftChar),
                               buildPosition(rightLine, rightChar)))
            .toEqual(expected);
      },
  );
});
