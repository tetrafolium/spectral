...
export type ComputeFingerprintFunc =
    (rule: IRuleResult, hash: (val: string) => string) => string;

export const defaultComputeResultFingerprint: ComputeFingerprintFunc =
    (rule, hash): string => {
...
export const prepareResults =
    (results: IRuleResult[], computeFingerprint: ComputeFingerprintFunc):
        IRuleResult[] => {
...
const deduplicateResults = (results: IRuleResult[]):
    IRuleResult[] => { return uniqBy([...results ], 'fingerprint'); };

const sortResults = (results: IRuleResult[]):
    IRuleResult[] => { return [...results ].sort(compareResults); };
