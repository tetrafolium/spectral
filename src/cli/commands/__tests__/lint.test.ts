...
function run(command: string) {
  ...
  return new Promise(done => {
    parser.parse(command, (err: Error, argv: unknown,
...
describe('lint', () => {
  ...
  describe('when STDIN is present', () => {
    it('does not show help when documents are missing', async () => {
      ...
      expect(output).not.toContain(
          'documents  Location of JSON/YAML documents');
    });
  ...
  it.each([ 'json', 'stylish' ])(
      'calls formatOutput with %s format', async format => {
        await run(`lint -f ${format} ./__fixtures__/empty-oas2-document.json`);
        // needed by Node 8 (different ticking?) - can be simplified once we
        // drop support for version 8
  ...
  it('writes formatted output to a file', async () => {
    ...
    // needed by Node 8 (different ticking?) - can be simplified once we drop
  ...
  it('passes skip-rule to lint', async () => {
    await run(
        'lint --skip-rule foo --skip-rule bar ./__fixtures__/empty-oas2-document.json');
  ...
  it('passes ignore-unknown-format to lint', async () => {
    await run(
        'lint --ignore-unknown-format ./__fixtures__/empty-oas2-document.json');
    expect(lint).toHaveBeenCalledWith([ expect.any(String) ], {
      encoding : 'utf8',
      format : 'stylish',
  ...
  it('passes show-unmatched-globs to lint', async () => {
    await run(
        'lint --show-unmatched-globs ./__fixtures__/empty-oas2-document.json');
    expect(lint).toHaveBeenCalledWith([ expect.any(String) ], {
      encoding : 'utf8',
      format : 'stylish',
      ignoreUnknownFormat : false,
  ...
  it('shows help if unknown format is passed', () => {
    return expect(run('lint -f foo ./__fixtures__/empty-oas2-document.json'))
        .resolves.toContain(
            'documents  Location of JSON/YAML documents. Can be either a file, a glob or',
        );
  });
  ...
  it('prints error message upon exception', async () => {
    ...
    (lint as jest.Mock).mockReturnValueOnce({
      // could be mockRejectedValueOnce, but Node 8 does not like it (different
      // ticking?), so here is the workaround
      then() { return this; },
      catch (fn: Function) {
        fn(error);
        return this;
