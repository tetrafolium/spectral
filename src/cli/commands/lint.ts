...
const lintCommand: CommandModule = {
  ...
  builder : yargs =>
      yargs.strict()
          ...
          .check((argv: Dictionary<unknown>) => {
            if (argv.format !== void 0 &&
                !(formatOptions as string[]).includes(String(argv.format))) {
              return false;
            }
  ...
  handler : args => {
    ...
    return lint(documents, {
             ...
             showUnmatchedGlobs,
             ruleset,
             ...pick<Partial<ILintConfig>, keyof ILintConfig>(
                 config, [ 'skipRule', 'verbose', 'quiet', 'resolver' ]),
           })
        .then(results => {
        ...
        .then(results => {
          if (results.length) {
            process.exitCode =
                severeEnoughToFail(results, failSeverity) ? 1 : 0;
          } else if (!config.quiet) {
            console.log(`No results with a severity of '${
                failSeverity}' or higher found!`);
          }
...
const filterResultsBySeverity =
    (results: IRuleResult[], failSeverity: FailSeverity): IRuleResult[] => {
      const diagnosticSeverity = getDiagnosticSeverity(failSeverity);
      return results.filter(r => r.severity <= diagnosticSeverity);
    };

export const severeEnoughToFail =
    (results: IRuleResult[], failSeverity: FailSeverity): boolean => {
      const diagnosticSeverity = getDiagnosticSeverity(failSeverity);
