...
const oas2PetstoreSpecPath =
    resolve(__dirname, '../../../__tests__/__fixtures__/petstore.oas2.json');
const validCustomOas3SpecPath =
    resolve(__dirname, '__fixtures__/openapi-3.0-valid-custom.yaml');
const invalidRulesetPath =
    resolve(__dirname, '__fixtures__/ruleset-invalid.yaml');
const validRulesetPath = resolve(__dirname, '__fixtures__/ruleset-valid.yaml');
const validNestedRulesetPath =
    resolve(__dirname, '__fixtures__/ruleset-extends-valid.yaml');
const invalidNestedRulesetPath =
    resolve(__dirname, '__fixtures__/ruleset-extends-invalid.yaml');
const standardOasRulesetPath =
    resolve(__dirname, '../../../../rulesets/oas/index.json');
const draftRefSpec = resolve(__dirname, './__fixtures__/draft-ref.oas2.json');
const draftNestedRefSpec =
    resolve(__dirname, './__fixtures__/draft-nested-ref.oas2.json');
const validOas3SpecPath =
    resolve(__dirname, './__fixtures__/openapi-3.0-valid.yaml');
const invalidOas3SpecPath =
    resolve(__dirname, '__fixtures__/openapi-3.0-no-contact.yaml');
...
function run(command: string) {
  ...
  const {documents, ...opts} =
      (parser.parse(command) as unknown) as ILintConfig &
      {documents : string[]};
...
describe('Linter service', () => {
  ...
  it('handles relative path to a document', async () => {
    const results =
        await run('lint src/__tests__/__fixtures__/gh-474/spec.yaml');
        ...
        expect.arrayContaining([
          expect.objectContaining({
            ...
            source : expect.stringContaining(
                '/__tests__/__fixtures__/gh-474/common.yaml'),
          }),
        ]),
    );
  });
  ...
  it('handles relative path to a document #2', async () => {
    const results =
        await run('lint src/__tests__/__fixtures__/gh-474/spec-2.yaml');
      ...
      describe('and --skip-rule=info-contact is set', () => {
        it('output other warnings but not info-contact', async () => {
          ...
          expect(output).toEqual(expect.arrayContaining(
              [ expect.objectContaining({code : 'oas3-api-servers'}) ]));
          expect(output).toEqual(expect.not.arrayContaining(
              [ expect.objectContaining({code : 'info-contact'}) ]));
        });
      });

      describe(
          'and --skip-rule=info-contact --skip-rule=oas3-api-servers is set',
          () => {
            it('outputs neither info-contact or oas3-api-servers', async () => {
              const output = await run(
                  `lint --skip-rule=info-contact --skip-rule=oas3-api-servers ${
                      document}`);
  ...
  describe('when a list of files is provided', () => {
    ...
    const documents = [
      join(process.cwd(),
           `src/__tests__/__fixtures__/petstore.invalid-schema.oas3.json`),
      join(process.cwd(), `src/__tests__/__fixtures__/petstore.oas3.json`),
    ];

    it('outputs issues for each file', () => {
      return expect(run([ 'lint', ...documents ].join(' ')))
          .resolves.toEqual(
              expect.arrayContaining([
                expect.objectContaining({
                  code : 'invalid-ref',
                ...
                expect.objectContaining({
                  code : 'invalid-ref',
                  path : [
                    'paths', '/pets', 'get', 'responses', 'default', 'content',
                    'application/json', 'schema', '$ref'
                  ],
                  source : join(
                      process.cwd(),
  ...
  describe('when glob is provided', () => {
    ...
    it('outputs issues for each file', () => {
      return expect(run(`lint ${documents}`))
          .resolves.toEqual(
              expect.arrayContaining([
                expect.objectContaining({
                  code : 'invalid-ref',
                ...
                expect.objectContaining({
                  code : 'oas3-schema',
                  path : [
                    'paths', '/pets', 'get', 'responses', '200', 'headers',
                    'header-1'
                  ],
                  source : join(
                      process.cwd(),
                ...
                expect.objectContaining({
                  ...
                  source :
                      join(process.cwd(),
                           'src/__tests__/__fixtures__/petstore.oas2.json'),
                }),
                expect.objectContaining({
                  code : 'operation-2xx-response',
                ...
                expect.objectContaining({
                  code : 'operation-description',
                  path : [
                    'paths', '/pet/{petId}/uploadImage', 'post', 'description'
                  ],
                  source :
                      join(process.cwd(),
                           'src/__tests__/__fixtures__/petstore.oas2.json'),
                ...
                expect.objectContaining({
                  code : 'operation-description',
                  path : [
                    'paths', '/user/createWithArray', 'post', 'description'
                  ],
                  source :
                      join(process.cwd(),
                           'src/__tests__/__fixtures__/petstore.oas2.json'),
                ...
                expect.objectContaining({
                  code : 'operation-description',
                  path : [
                    'paths', '/user/createWithList', 'post', 'description'
                  ],
                  source :
                      join(process.cwd(),
                           'src/__tests__/__fixtures__/petstore.oas2.json'),
    ...
    it('unixifies patterns', () => {
      return expect(
                 run(`lint src\\__tests__\\__fixtures__\\petstore.invalid-schema.*.json`))
          .resolves.toEqual(
              expect.arrayContaining([
                expect.objectContaining({
                  code : 'invalid-ref',
  ...
  describe('--ruleset', () => {
    describe('extends feature', () => {
      it('extends a valid relative ruleset', () => {
        return expect(run(`lint ${validCustomOas3SpecPath} -r ${
                          validNestedRulesetPath}`))
            .resolves.toEqual([]);
      });

      it('fails trying to extend an invalid relative ruleset', () => {
        return expect(run(`lint ${validCustomOas3SpecPath} -r ${
                          invalidNestedRulesetPath}`))
      ...
      it('given remote nested ruleset, resolves', () => {
        ...
        return expect(
                   run(`lint ${
                       validCustomOas3SpecPath} -r http://foo.local/ruleset-master.yaml`))
            .resolves.toEqual(
                [],
            );
      });
    ...
    describe('when multiple ruleset options provided', () => {
      it('given one is valid other is not, outputs "invalid ruleset" error',
         () => {
           return expect(
                      run(`lint ${validCustomOas3SpecPath} -r ${
                          invalidRulesetPath} -r ${validRulesetPath}`),
                      )
    ...
    describe('when single ruleset option provided', () => {
      it('outputs "does not exist" error', () => {
        return expect(run(`lint ${validOas3SpecPath} -r non-existent-path`))
            .rejects.toThrow(
      ...
      it('outputs "invalid ruleset" error', () => {
        return expect(run(`lint ${validOas3SpecPath} -r ${invalidRulesetPath}`))
            .rejects.toThrowError(ValidationError);
      });
      ...
      it('outputs warnings', async () => {
        const output =
            await run(`lint ${validOas3SpecPath} -r ${validRulesetPath}`);
        expect(output).toEqual(expect.arrayContaining(
            [ expect.objectContaining({code : 'info-matches-stoplight'}) ]));
        expect(output).toEqual(
            expect.not.arrayContaining([
              expect.objectContaining({
      ...
      it('given valid remote ruleset file, outputs no issues', () => {
        ...
        return expect(
                   run(`lint ${
                       validCustomOas3SpecPath} -r http://foo.local/ruleset.yaml`))
            .resolves.toEqual([]);
      });

      describe('when a standard oas3 ruleset provided through option', () => {
        it('outputs warnings', () => {
          return expect(run(`lint ${invalidOas3SpecPath} -r ${
                            standardOasRulesetPath}`))
              .resolves.toEqual(
                  expect.arrayContaining([
                    expect.objectContaining({code : 'oas3-api-servers'}),
                    expect.objectContaining({code : 'info-contact'}),
      ...
      describe('when a standard oas2 ruleset provided through option', () => {
        it('outputs warnings', async () => {
          const output = await run(
              `lint ${oas2PetstoreSpecPath} -r ${standardOasRulesetPath}`);
          expect(output).toEqual(expect.arrayContaining(
              [ expect.objectContaining({code : 'operation-description'}) ]));
  ...
  describe('when loading specification files from web', () => {
    ...
    it('throws if cannot load URI', () => {
      ...
      return expect(run('lint http://foo.local/openapi'))
          .rejects.toThrow(
  ...
  describe('when using default ruleset file', () => {
    ...
    beforeAll(() => {
      spy = jest.spyOn(process, 'cwd')
                .mockReturnValue(resolve(__dirname, '__fixtures__'));
    });
    ...
    it('outputs errors occurring in referenced files', () => {
      return expect(run(`lint ${draftRefSpec}`)).resolves.toEqual([
        expect.objectContaining({
          ...
          source : expect.stringContaining(
              '__tests__/__fixtures__/draft-ref.oas2.json'),
        }),
        expect.objectContaining({
          code : 'openapi-tags',
        ...
        expect.objectContaining({
          ...
          source :
              expect.stringContaining('/__tests__/__fixtures__/refs/info.json'),
        }),
        expect.objectContaining({
          code : 'info-description',
          message :
    ...
    it('outputs errors occurring in nested referenced files', () => {
      return expect(run(`lint ${draftNestedRefSpec}`)).resolves.toEqual([
        expect.objectContaining({
          ...
          range : expect.any(Object),
          source : expect.stringContaining(
              '__tests__/__fixtures__/draft-nested-ref.oas2.json'),
        }),
        expect.objectContaining({
          code : 'openapi-tags',
          message : 'OpenAPI object should have non-empty `tags` array.',
        ...
        expect.objectContaining({
          ...
          source : expect.stringContaining(
              '__tests__/__fixtures__/refs/contact.json'),
        }),
        expect.objectContaining({
          code : 'info-description',
          message :
        ...
        expect.objectContaining({
          ...
          source :
              expect.stringContaining('__tests__/__fixtures__/refs/paths.json'),
        }),
        expect.objectContaining({
          code : 'operation-description',
          message :
  ...
  describe('--resolver', () => {
    it('uses provided resolver for $ref resolving', async () => {
      expect(await run(`lint --resolver ${fooResolver} ${fooDocument}`))
          .toEqual(
              expect.arrayContaining([
                expect.objectContaining({
                  code : 'info-contact',
