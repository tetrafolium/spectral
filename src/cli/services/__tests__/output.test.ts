...
describe('Output service', () => {
  describe('formatOutput', () => {
    it.each([ 'stylish', 'json',
              'junit' ])('calls %s formatter with given result', format => {
      const results = [
        {
          code : 'info-contact',
          path : [ 'info' ],
  ...
  describe('writeOutput', () => {
    ...
    it('given outputFile, writes output to a specified path', async () => {
      ((fs.writeFile as any) as jest.Mock)
          .mockImplementationOnce((path, val, cb) => { cb(null, void 0); });

      const output = '{}';
      const outputFile = 'foo.json';
      expect(await writeOutput(output, outputFile)).toBeUndefined();
      expect(fs.writeFile)
          .toBeCalledWith(outputFile, output, expect.any(Function));
    });
