...
describe('listFiles CLI util', () => {
  it('unixify paths', () => {
    listFiles(
        [
          '.\\repro\\lib.yaml', './foo/*.json',
          '.\\src\\__tests__\\__fixtures__\\*.oas.json'
        ],
        true);
  ...
  it('returns file paths', async () => {
    const list = [
      path.join(__dirname, 'foo/a.json'), path.join(__dirname, 'foo/b.json')
    ];
  ...
  it('given disabled ignoredUnmatchedGlobs, reports unmatched patterns',
     async () => {
       const list = [
         path.join(__dirname, 'foo/a.json'), path.join(__dirname, 'foo/b.json')
       ];
       ...
       expect(await listFiles([ './foo/*.json', 'bar/**/baz*.yaml' ], false))
           .toEqual([ list, [ 'bar/**/baz*.yaml' ] ]);
     });
});
