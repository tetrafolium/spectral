...
export async function listFiles(
  ...
  const {files, fileDescriptors, urls} = pattens.reduce<
      {files : string[]; urls : string[]; fileDescriptors : number[];}>(
      (group, pattern) => {
  ...
  return [
    [...urls, ...fileDescriptors, ...filesFound ], fileSearchWithoutResult
  ]; // let's normalize OS paths produced by fast-glob to have consistent paths
