...
describe('schema', () => {
  ...
  describe('when schema defines unknown format', () => {
    ...
    beforeEach(() => { jest.spyOn(console, 'warn'); });
    ...
    describe('and an enum consisting of string values', () => {
      ...
      it('reports pretty enum errors for a string', () => {
        expect(runSchema('baz', testSchema)).toEqual([
          {
            message :
                `String should be equal to one of the allowed values: foo, bar. Did you mean bar?`,
            path : [],
          },
        ]);
      });
    ...
    describe('and an enum consisting of integer values', () => {
      ...
      it('reports pretty enum errors for a number', () => {
        expect(runSchema(2, testSchema)).toEqual([
          {
            message :
                `Number should be equal to one of the allowed values: 1, 3, 5, 10, 12`,
            path : [],
          },
        ]);
      });
  ...
  test(
      'reports slightly less pretty enum errors for primitive values that are not similar to any values in enum',
      () => {
        const testSchema: JSONSchema6 = {
          $schema : `http://json-schema.org/draft-06/schema#`,
          type : 'string',
        ...
        expect(runSchema('three', testSchema)).toEqual([
          {
            message :
                `String should be equal to one of the allowed values: foo, bar`,
            path : [],
          },
        ]);
      });
  ...
  describe('when schema has a $ref left', () => {
    test('given unresolved context, reports an error', () => {
      expect(
          runSchema({}, {$ref : '#/foo'}, void 0, {rule : {resolved : false}}))
          .toEqual([
            {
    ...
    test('given resolved context, ignores', () => {
      expect(runSchema({}, {$ref : '#/bar'}, void 0, {
        rule : {resolved : true}
      })).toEqual([]);
    });
  });
