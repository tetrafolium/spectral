...
function runCasing(target: unknown, type: CasingType, disallowDigits?: boolean,
                   separator?: any) {
  return casing(
      target,
...
describe('casing', () => {
  ...
  describe('casing type', () => {
    describe('flat', () => {
      ...
      test.each(invalid)('should recognize invalid target %s', target => {
        expect(runCasing(target, CasingType.flat)).toEqual([
          {message : 'must be flat case'}
        ]);
      });

      test.each([...valid, ...validWithDigits ])(
          'should recognize valid target %s', target => {
            expect(runCasing(target, CasingType.flat)).toBeUndefined();
          });

      describe('when digits are disallowed', () => {
        test.each([...invalid, ...validWithDigits ])(
            'should recognize invalid target %s', target => {
              expect(runCasing(target, CasingType.flat, true)).toEqual([
                {message : 'must be flat case'}
              ]);
            });
    ...
    describe('camel', () => {
      ...
      test.each(invalid)('should recognize invalid target %s', target => {
        expect(runCasing(target, CasingType.camel)).toEqual([
          {message : 'must be camel case'}
        ]);
      });
      ...
      describe('when digits are disallowed', () => {
        test.each([...invalid, ...validWithDigits ])(
            'should recognize invalid target %s', target => {
              expect(runCasing(target, CasingType.camel, true)).toEqual([
                {message : 'must be camel case'}
              ]);
            });
    ...
    describe('pascal', () => {
      const invalid = [
        'foo_test', '123', '1fooBarBaz', 'fooBarBaz1', 'fooBar', 'foo1',
        'foo-bar'
      ];
      const valid = [ 'Foo', 'FooBar', 'FooBarBaz' ];
      ...
      test.each(invalid)('should recognize invalid target %s', target => {
        expect(runCasing(target, CasingType.pascal)).toEqual([
          {message : 'must be pascal case'}
        ]);
      });
      ...
      describe('when digits are disallowed', () => {
        test.each([...invalid, ...validWithDigits ])(
            'should recognize invalid target %s', target => {
              expect(runCasing(target, CasingType.pascal, true)).toEqual([
                {message : 'must be pascal case'}
              ]);
            });
    ...
    describe('kebab', () => {
      ...
      test.each(invalid)('should recognize invalid target %s', target => {
        expect(runCasing(target, CasingType.kebab)).toEqual([
      ...
      describe('when digits are disallowed', () => {
        test.each([...invalid, ...validWithDigits ])(
            'should recognize invalid target %s', target => {
              expect(runCasing(target, CasingType.kebab, true)).toEqual([
                {message : 'must be kebab case'}
              ]);
            });
    ...
    describe('cobol', () => {
      const invalid = [
        'foo_test', 'Foo1', '123', 'fooBarBaz1', 'FOo', 'FOO-BAr', 'FOO--BAR',
        'FOO-', '-FOO'
      ];
      const valid = [ 'FOO', 'FOO-BAR', 'FOO-BAR-BAZ' ];
      ...
      test.each(invalid)('should recognize invalid target %s', target => {
        expect(runCasing(target, CasingType.cobol)).toEqual([
          {message : 'must be cobol case'}
        ]);
      });
      ...
      describe('when digits are disallowed', () => {
        test.each([...invalid, ...validWithDigits ])(
            'should recognize invalid target %s', target => {
              expect(runCasing(target, CasingType.cobol, true)).toEqual([
                {message : 'must be cobol case'}
              ]);
            });
    ...
    describe('snake', () => {
      const invalid = [
        'Foo1', '123', 'fooBarBaz1', 'FOo', 'FOO-BAR', 'foo__bar', '1foo_bar1',
        'foo_', '_foo'
      ];
      ...
      test.each(invalid)('should recognize invalid target %s', target => {
        expect(runCasing(target, CasingType.snake)).toEqual([
          {message : 'must be snake case'}
        ]);
      });
      ...
      describe('when digits are disallowed', () => {
        test.each([...invalid, ...validWithDigits ])(
            'should recognize invalid target %s', target => {
              expect(runCasing(target, CasingType.snake, true)).toEqual([
                {message : 'must be snake case'}
              ]);
            });
    ...
    describe('macro', () => {
      ...
      test.each(invalid)('should recognize invalid target %s', target => {
        expect(runCasing(target, CasingType.macro)).toEqual([
          {message : 'must be macro case'}
        ]);
      });
      ...
      describe('when digits are disallowed', () => {
        test.each([...invalid, ...validWithDigits ])(
            'should recognize invalid target %s', target => {
              expect(runCasing(target, CasingType.macro, true)).toEqual([
                {message : 'must be macro case'}
              ]);
            });
  ...
  describe('casing with custom separator configuration', () => {
    const baseData: Array<[ string, string, string, string ]> =
        [ [ CasingType.flat, 'flat', 'flat01', 'Nope' ] ];

    const testCases:
        Array<[ string, boolean, string, boolean, string, string, string ]> =
            [];
    Object.values(baseData).forEach(
        data => ['/', '*', '-'].forEach(
            char => [true, false].forEach(
                allowLeading => [true, false].forEach(
                    disallowDigits => testCases.push([
                      data[0], disallowDigits, char, allowLeading, data[1],
                      data[2], data[3]
                    ]),
                    ),
                ),
    ...
    describe('properly detects valid cases', () => {
      test.each(testCases)(
          ...
          (type, disallowDigits, char, allowLeading, simple, withDigits,
           invalid) => {
            expect(
                runCasing(`${simple}${char}${simple}`, CasingType[type],
                          disallowDigits, {char, allowLeading}),
                )
                .toBeUndefined();

            expect(
                runCasing(`${simple}${char}${invalid}`, CasingType[type],
                          disallowDigits, {char, allowLeading}),
                )
                .toEqual([ {message : `must be ${type} case`} ]);

            const digitsResults = runCasing(`${withDigits}${char}${simple}`,
                                            CasingType[type], disallowDigits, {
                                              char,
                                              allowLeading,
                                            });
            ...
            if (!disallowDigits) {
              ...
              expect(digitsResults).toEqual([
                {message : `must be ${type} case`}
              ]);
            }

            const leadingSepResults =
                runCasing(`${char}${simple}${char}${simple}`, CasingType[type],
                          disallowDigits, {
                            char,
                            allowLeading,
                          });
    ...
    test.each(Object.values(
        CasingType))('properly detects leading char for %s casing', casing => {
      expect(runCasing('/', casing, true, {allowLeading : true, char : '/'}))
          .toBeUndefined();
      expect(runCasing('//', casing, true, {allowLeading : true, char : '/'}))
          .toEqual([
    ...
    test('allows advanced scenarios', () => {
      expect(runCasing('X-MyAmazing-Header', CasingType.pascal, true, {
        char : '-'
      })).toBeUndefined();
      expect(
          runCasing('/path/to/myResource', CasingType.camel, true,
                    {char : '/', allowLeading : true}),
          )
          .toBeUndefined();
    });
  });
