...
function runAlphabetical(target: any, keyedBy?: string) {
  return alphabetical(
      ...
      {given : [ '$' ]},
      {
        given : null,
        original : null,
        documentInventory : new DocumentInventory(
            new Document(safeStringify(target), Parsers.Json), {} as any),
        rule : {} as any,
      },
  );
}
  ...
  test('given an object and keys not in order return an error message', () => {
    expect(
        ...
        .toEqual([
          {
            message :
                'at least 2 properties are not in alphabetical order: "c" should be placed after "b"',
            path : [ '$', 'c' ],
          },
        ]);
  });
...
'200':
  description: ''`).data;
    ...
    expect(runAlphabetical(doc)).toEqual([
      {
        message :
            'at least 2 properties are not in alphabetical order: "400" should be placed after "200"',
        path : [ '$', '400' ],
      },
    ]);
  });
  ...
  describe('given NO keyedBy', () => {
    test('given an unsorted array of strings should return error', () => {
      expect(runAlphabetical([ 'b', 'a' ])).toEqual([
        {
          message :
              'at least 2 properties are not in alphabetical order: "b" should be placed after "a"',
          path : [ '$', 0 ],
        },
      ]);
    });
    ...
    test('given an unsorted array of numbers should return error', () => {
      expect(runAlphabetical([ 10, 1 ])).toEqual([
        {
          message :
              'at least 2 properties are not in alphabetical order: "10" should be placed after "1"',
          path : [ '$', 0 ],
        },
      ]);
    });
  ...
  describe('given keyedBy', () => {
    ...
    test(
        'given an array of objects with sorted prop values to NOT return an error',
        () => {
          expect(runAlphabetical([ {a : '1'}, {a : '2'}, {a : '2'} ], 'a'))
              .toBeUndefined();
        });
  ...
  test('is able to trap object again', () => {
    ...
    Object.defineProperty(document, 'data', {
      value : Object.defineProperties(
          {}, Object.getOwnPropertyDescriptors(document.data)),
    });
    ...
    expect(
        ...
        .toEqual([
          {
            message :
                'at least 2 properties are not in alphabetical order: "404" should be placed after "200"',
            path : [ '$', '404' ],
          },
        ]);
  });
