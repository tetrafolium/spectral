...
describe('pattern', () => {
  describe('given a true regex', () => {
    test(
        'should return empty array when given value matches the given match regex',
        () => { expect(runPattern('abc', {match : /[abc]+/})).toEqual([]); });

    test(
        'should return a message when given value does not match the given notMatch regex',
        () => {
          expect(runPattern('def', {match : /[abc]+/})).toEqual([
            {message : "must match the pattern '/[abc]+/'"}
          ]);
        });

    test(
        'should return empty array when given value does not match the given notMatch regex',
        () => {
          expect(runPattern('dEf', {notMatch : /[abc]+/i})).toEqual([]);
        });

    test(
        'should return a message when given value does match the given notMatch regex',
        () => {
  ...
  describe('given a string regex', () => {
    test(
        'should return empty array when given value matches the given match string regex without slashes',
        () => { expect(runPattern('abc', {match : '[abc]+'})).toEqual([]); });

    test(
        'should return empty array when given value matches the given match string regex with slashes',
        () => { expect(runPattern('abc', {match : '/[abc]+/'})).toEqual([]); });
    ...
    test(
        'should throw an exception when given string regex contains invalid flags',
        () => {
          expect(() => runPattern('aBc', {match : '/[abc]+/invalid'}))
              .toThrow(
  ...
  describe('given match and notMatch regexes', () => {
    test(
        ...
        () => {
          expect(runPattern('def', {match : /[def]+/, notMatch : /[abc]+/}))
              .toEqual([]);
        });
  });
});
