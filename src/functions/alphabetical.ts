...
const compare = (a: unknown, b: unknown): number => {
  if ((typeof a === 'number' || Number.isNaN(Number(a))) &&
      (typeof b === 'number' || !Number.isNaN(Number(b)))) {
...
const getUnsortedItems =
    <T>(arr: T[], compareFn: (a: T, B: T) => number): null|[number, number] => {
...
export const alphabetical: IFunction<IAlphaRuleOptions|
                                     null> = (targetVal, opts, paths,
                                              {documentInventory}) => {
  ...
  if (Array.isArray(targetVal)) {
    ...
    targetVal = documentInventory.findAssociatedItemForPath(paths.given, true)
        ?.document.trapAccess(targetVal)
        ? ? targetVal;
    ...
    return [
      {
        ...(!keyedBy && {
          path : [
            ...path, Array.isArray(targetVal) ? unsortedItems[0]
        ...
        message :
            keyedBy
                ...
                : `at least 2 properties are not in alphabetical order: "${
                      targetArray[unsortedItems[0]]}" should be placed after "${
