...
export interface ISchemaFunction extends IFunction<ISchemaOptions> {
  Ajv: typeof AJV;
  specs:
      {v4: typeof jsonSpecV4; v6 : typeof jsonSpecV6; v7 : typeof jsonSpecV7;};
...
function getAjv(oasVersion: Optional<number>,
                allErrors: Optional<boolean>): AJV.Ajv {
  const type: string = oasVersion !== void 0 && oasVersion >= 2
                           ? 'oas' + oasVersion
                           : 'jsonschema';
...
const validators = new (class extends WeakMap<JSONSchema, ValidateFunction>{
  public get({schema : schemaObj, oasVersion, allErrors}: ISchemaOptions) :
    ...
    export const schema: ISchemaFunction =
        (targetVal, opts, paths, {rule}) => {
          ...
          try {
            // we used the compiled validation now, hence this lookup here (see
            // the logic above for more info)
            const validator = opts.ajv ? ? validators.get(opts);
            if (validator(targetVal) === false &&
              ...
              results.push(
                  ...betterAjvErrors(schemaObj, validator.errors, {
                    ...
                    targetValue : targetVal,
                           ...
                           message : suggestion !== void 0
                                         ? `${error}. ${suggestion}`
                                         : error,
                           path : [
                             ...path,
                             ...(errorPath !== ''
                                     ? errorPath.replace(/^\//, '').split('/')
                                     : [])
                           ],
                         })),
              );
            ...
            if (!(ex instanceof AJV.MissingRefError)) {
              ...
              // let's ignore any $ref errors if schema fn is provided with
              // already resolved content, if our resolver fails to resolve
              // them, ajv is unlikely to do it either, since it won't have
              // access to the whole document, but a small portion of it
              results.push({
                message : ex.message,
                path,
              });
    ...
    schema.createAJVInstance =
        (opts: AJV.Options): AJV.Ajv => {
          const ajv = new AJV(opts);

          ajv.addFormat('int32',
                        {type : 'number', validate : oasFormatValidator.int32});
          ajv.addFormat('int64',
                        {type : 'number', validate : oasFormatValidator.int64});
          ajv.addFormat('float',
                        {type : 'number', validate : oasFormatValidator.float});
          ajv.addFormat(
              'double',
              {type : 'number', validate : oasFormatValidator.double});
          ajv.addFormat('byte',
                        {type : 'string', validate : oasFormatValidator.byte});
