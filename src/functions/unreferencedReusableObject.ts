...
export const unreferencedReusableObject:
    IFunction<{reusableObjectsLocation : string}> = (
        data,
        opts,
        _paths,
        otherValues,
      ...
      if (graph === null) {
        return [
          {message : 'unreferencedReusableObject requires dependency graph'}
        ];
      }

      const normalizedSource = otherValues.documentInventory.source ? ? '';

      const defined = Object.keys(data).map(
          name => `${normalizedSource}${opts.reusableObjectsLocation}/${name}`);
