...
const splitWord = (word: string, end: string, start: string) =>
...
export function getDiagnosticErrorMessage(diagnostic: IDiagnostic) {
  ...
  if (key !== undefined) {
    prettifiedMessage =
        prettifiedMessage.replace(/(Duplicate key)/, `$1: ${key}`);
  }

  return prettifiedMessage;
}

export const prettyPrintResolverErrorMessage = (message: string) =>
    message.replace(/^Error\s*:\s*/, '');
...
export function formatParserDiagnostics(diagnostics: ReadonlyArray<IDiagnostic>,
                                        source: string|null): IRuleResult[] {
...
export const formatResolverErrors =
    (document: IDocument, diagnostics: IResolveError[]): IRuleResult[] => {
      return uniqBy(diagnostics, 'message').map<IRuleResult>(error => {
        ...
        const source = error.uriStack.length > 0
                           ? error.uriStack[error.uriStack.length - 1]
                           : document.source;
