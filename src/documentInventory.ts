...
import {
  ...
  IGraphNodeData,
  IUriParser
} from '@stoplight/json-ref-resolver/types';
...
export type DocumentInventoryItem = {
  document: IDocument; path : JsonPath; missingPropertyPath : JsonPath;
};

export class DocumentInventory {
  private static readonly _cachedRemoteDocuments =
      new WeakMap<ICache|IResolver, Dictionary<Document>>();
  ...
  public get source(): string|null { return this.document.source; }

  public get unresolved(): unknown { return this.document.data; }

  public get formats() { return this.document.formats; }

  constructor(public readonly document: IDocument<unknown>,
              protected resolver: IResolver) {
    this.graph = null;
    this.errors = null;

    const cacheKey =
        resolver instanceof Resolver ? resolver.uriCache : resolver;
    const cachedDocuments =
        DocumentInventory._cachedRemoteDocuments.get(cacheKey);
    if (cachedDocuments !== void 0) {
      this.referencedDocuments = cachedDocuments;
    } else {
      this.referencedDocuments = {};
      DocumentInventory._cachedRemoteDocuments.set(cacheKey,
                                                   this.referencedDocuments);
    }
  }
  ...
  public findAssociatedItemForPath(path: JsonPath, resolved: boolean):
      DocumentInventoryItem|null {
    if (!resolved) {
    ...
    try {
      ...
      const missingPropertyPath =
          newPath.length === 0
              ? []
              : path.slice(path.lastIndexOf(newPath[newPath.length - 1]) + 1);
      ...
      while (true) {
        ...
        if (isLocalRef($ref)) {
          resolvedDoc = source === this.document.source
                            ? this.document
                            : this.referencedDocuments[source];
        } else {
          const extractedSource = extractSourceFromRef($ref)!;
          source = isAbsoluteRef(extractedSource)
                       ? extractedSource
                       : resolve(source, '..', extractedSource);

          resolvedDoc = source === this.document.source
                            ? this.document
                            : this.referencedDocuments[source];
          const obj = scopedPath.length === 0 || hasRef(resolvedDoc.data)
                          ? resolvedDoc.data
                          : get(resolvedDoc.data, scopedPath);
  ...
  protected parseResolveResult = async (resolveOpts: IUriParser) => {
    ...
    const parser: IParser<IParserResult<unknown, any, any, any>> =
    ...
    if (document.diagnostics.length > 0) {
      this.diagnostics.push(
          ...formatParserDiagnostics(document.diagnostics, document.source));
    }
