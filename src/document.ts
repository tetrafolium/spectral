...
export function normalizeSource(source: Optional<string>): string|null {
  if (source === void 0)
    return null;
  return source.length > 0 && !startsWithProtocol(source) ? normalize(source)
                                                          : source;
}
...
export class Document<D = unknown, R extends IParserResult = IParserResult<D>>
  ...
  constructor(protected readonly input: string,
              protected readonly parser: IParser<R>, source?: string) {
    this.parserResult = parser.parse(input);
    // we need to normalize the path in case path with forward slashes is given
    this.source = normalizeSource(source);
    this.diagnostics =
        formatParserDiagnostics(this.parserResult.diagnostics, this.source);
  }

  public getRangeForJsonPath(path: JsonPath,
                             closest?: boolean): Optional<IRange> {
    return this.parser.getLocationForJsonPath(this.parserResult, path, closest)
        ?.range;
  }
  ...
  public get data() { return this.parserResult.data; }
}

export class ParsedDocument<D = unknown,
                            R extends IParsedResult = IParsedResult> implements
    IDocument<D> {
  public readonly source: string|null;
  public readonly diagnostics: ReadonlyArray<IRuleResult>;
  public formats?: string[]|null;
  ...
  constructor(protected readonly parserResult: R) {
    ...
    this.diagnostics = formatParserDiagnostics(
        this.parserResult.parsed.diagnostics, this.source);
  }

  public trapAccess<T extends object = object>(obj: T): T { return obj; }

  public getRangeForJsonPath(path: JsonPath,
                             closest?: boolean): Optional<IRange> {
    return this.parserResult.getLocationForJsonPath(this.parserResult.parsed,
                                                    path, closest)
        ?.range;
  }

  public get data() { return this.parserResult.parsed.data; }
}

export interface IParsedResult<
    R extends IParserResult = IParserResult<unknown, any, any, any>> {
