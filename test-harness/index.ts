...
const files = process.env.TESTS ? String(process.env.TESTS).split(',')
                                : fg.sync('**/*.scenario', {cwd});
...
describe('cli acceptance tests', () => {
  describe.each(files)('%s file', file => {
    ...
    beforeAll(async () => {
      await Promise.all(
          scenario.assets.map(async ([ asset ]) => {
            ...
            replacements[`${asset}|no-ext`] = normalizedName.replace(
                new RegExp(`${path.extname(normalizedName)}$`), '');
            replacements[`${asset}|filename|no-ext`] =
                path.basename(normalizedName, true);
          }),
      );
    ...
    test(scenario.test, async () => {
      ...
      replacements.date =
          String(new Date()); // this may introduce random failures, but
                              // hopefully they don't occur too often

      const expectedStdout =
          scenario.stdout === void 0
              ? void 0
              : applyReplacements(scenario.stdout, replacements);
      const expectedStderr =
          scenario.stderr === void 0
              ? void 0
              : applyReplacements(scenario.stderr, replacements);
